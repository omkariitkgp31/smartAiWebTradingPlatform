package main

import (
	"log"
	"net/http"

	"github.com/gin-contrib/cors"
	"github.com/gin-gonic/gin"

	"github.com/stockbroker/queue-processor/config"
	"github.com/stockbroker/queue-processor/database"
	"github.com/stockbroker/queue-processor/dispatcher"
	"github.com/stockbroker/queue-processor/handlers"
	"github.com/stockbroker/queue-processor/models"
)

func main() {
	cfg := config.Load()
	dispatcher.Init(cfg)
	database.Connect(cfg)
	models.AutoMigrate(database.DB)

	r := gin.Default()
	r.Use(cors.Default())

	r.GET("/", func(c *gin.Context) {
		c.JSON(http.StatusOK, gin.H{"service": cfg.AppName, "version": "1.0.0"})
	})
	r.GET("/health", func(c *gin.Context) {
		c.JSON(http.StatusOK, gin.H{"status": "healthy", "service": "queue-processor"})
	})

	r.POST("/buy", handlers.ProcessBuy)
	r.POST("/sell", handlers.ProcessSell)
	r.GET("/events", handlers.ListEvents)

	log.Printf("🚀 %s running on :%s", cfg.AppName, cfg.AppPort)
	r.Run(":" + cfg.AppPort)
}
